    ...
    <>
      ...
      <Group>
        <Link to={`/dashboard/apps/${app.id}`}>
          <Button variant="light">General</Button>
        </Link>
        <Link to={`/dashboard/apps/${app.id}/licenses`}>
          <Button variant="light">View Licenses</Button>
        </Link>
      </Group>
      <Divider my={20} />
      ...
      <Modal opened={opened} onClose={close} title="Are you sure?">
        <Form method="delete">
          <TextInput
            data-autofocus
            label={`Please type "${app.name}" to continue`}
            value={deleteValue}
...
const EditNameButton = ({ currentName }: { currentName: string }) => {
  ...
  return (
    <>
      ...
      <Modal opened={open} onClose={close} title="Rename App">
        <Form method="patch">
          <Stack>
            <TextInput
              data-autofocus
